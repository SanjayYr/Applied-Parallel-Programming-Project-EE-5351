...
__global__ void mean_kernel(float* phid, unsigned int* datain_d, unsigned int height, unsigned int width, unsigned int* sum_neg_d, unsigned int* sum_pos_d, int* c_neg_d, int* c_pos_d)
{
	__shared__ unsigned int s_sum_neg;
	__shared__ unsigned int s_sum_pos;
	...
	if (tid < height*width)
	{
		if (phid[tid] <= 0)
		{
			atomicAdd(&s_sum_neg, datain_d[tid]);
			atomicAdd(&s_c_neg, 1);
		}
		else
		{
			atomicAdd(&s_sum_pos, datain_d[tid]);
			atomicAdd(&s_c_pos, 1);
		}
		...
		if (tx == 0)
		{
			...
			atomicAdd(sum_neg_d, s_sum_neg);
			atomicAdd(c_neg_d, s_c_neg);
			atomicAdd(sum_pos_d, s_sum_pos);
