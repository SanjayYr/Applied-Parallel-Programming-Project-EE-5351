...
//	gettimeofday(&t1, NULL);
	...
	while (its < max_its && ~stop)
	{
     //   cudaMemcpy(phid, phi, height*width*sizeof(float), cudaMemcpyHostToDevice);
    //    cudaMemcpy(datain_d, datain, height*width*sizeof(unsigned int), cudaMemcpyHostToDevice);
		//cudaMemset(mean_neg_d, 0, sizeof(unsigned int));
		//cudaMemset(mean_pos_d, 0, sizeof(unsigned int));
		//cudaMemset(c_neg_d, 0, sizeof(int));
		//cudaMemset(c_pos_d, 0, sizeof(int));
 ...
 // kernel mean start
		...
		// copy back to host
		//cudaMemcpy(&sum_neg, mean_neg_d, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		//cudaMemcpy(&sum_pos, mean_pos_d, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		//cudaMemcpy(&c_neg, c_neg_d, sizeof(int), cudaMemcpyDeviceToHost);
		//cudaMemcpy(&c_pos, c_pos_d, sizeof(int), cudaMemcpyDeviceToHost);

		//mean_neg = sum_neg / ((float)c_neg + EPS);
		//mean_pos = sum_pos / ((float)c_pos + EPS);
  // kernel mean finish
...
/*
	...
	*/
	...
	// kernel force start 
		//cudaMemset(maxF_d, 0, sizeof(double));

        
        kernel_call_force(phidd, datain_d,F_d, height, width, maxF_d, stop_d, mean_neg_d, mean_pos_d, c_neg_d, c_pos_d);
		// copy back to host

//        cudaMemcpy(F, F_d, height*width*sizeof(float), cudaMemcpyDeviceToHost);

	//	cudaMemcpy(&max_F, maxF_d, sizeof(double), cudaMemcpyDeviceToHost);
...
*/
	...
	curvature_kernel_call(curvature_d, phidd, height,width);
//	cudaMemcpy(curvature, curvature_d, height*width*sizeof(float), cudaMemcpyDeviceToHost);

                
                //cudaMemset(max_dphidt_d, 0, sizeof(double));
                //cudaMemcpy(curvature_d, curvature, height*width*sizeof(float), cudaMemcpyHostToDevice);

     gradient_kernel_call(phidd, curvature_d, F_d, dphidt_d, height, width, max_dphidt_d, 0.2, maxF_d); // Function call to launch the kernel from the function

                // Copy the results back to the host
                //cudaMemcpy(dphidt, dphidt_d, height*width*sizeof(float), cudaMemcpyDeviceToHost);
                //cudaMemcpy(&max_dphidt, max_dphidt_d, sizeof(double), cudaMemcpyDeviceToHost);               
    // gradient kernel end
...
*/
  ...
  //CLF kernel start
  //              float *phidCFL;
  //              cudaMalloc((void**)&phidCFL, height * width * sizeof(float));
  //              cudaMemcpy(phidCFL, phi, height*width*sizeof(float), cudaMemcpyHostToDevice);
                CFL_kernel_call(phid, phidd, dphidt_d, height, width, max_dphidt_d); // Function call to launch the kernel

                // Copy results back to the host
         //       cudaMemcpy(phi, phidCFL, height*width*sizeof(float), cudaMemcpyDeviceToHost);
		// CLF kernel end
...
*/
	...
	//	cudaMemcpy(phid, phi, height*width*sizeof(float), cudaMemcpyHostToDevice);
		...
		// copy back to host
		//cudaMemcpy(phi, phidd, height*width * sizeof(float), cudaMemcpyDeviceToHost);
